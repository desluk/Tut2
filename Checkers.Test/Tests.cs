...
namespace Checkers.Test
{
        ...
        static object[] legalMovesOnEmptyBoard =
  {
        ...
        public void APieceKnowsTheLegalMovesItCanMake(Color c, int pos, int[] expected)
        {
            ...
            IBoard b = Substitute.For<IBoard>(); // this creates a the IBoard mock
            b.Pieces(c).Returns(new int[] { pos }); // this should return a single pos when given c this pos is where the piece should be found
            b.Pieces(Arg.Is<Color>(x => x != c)).Returns(Array.Empty<int>()); // this says that when not the same as c than give back empty array
            // this should then simulate a board.
